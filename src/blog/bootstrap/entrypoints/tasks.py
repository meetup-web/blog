...
from blog.bootstrap.container import bootstrap_worker_container
from blog.infrastructure.outbox.adapters.rabbitmq_outbox_publisher import (
    ExchangeName,
)
from blog.infrastructure.outbox.process_outbox_cron_task import (
    process_outbox,
)
from blog.infrastructure.persistence.sql_tables import map_outbox_table
...
async def bind_queue_to_exchange(state: TaskiqState) -> None:
    await state.faststream_broker.declare_exchange(
        RabbitExchange(name=ExchangeName.BLOG, durable=True, type=ExchangeType.DIRECT)
    )
